...
public class FrameControllerAdvice {
	...
	public JsonResult errorHandler(ConstraintViolationException ex) {
		...
		return new JsonResult(JsonResult.ERROR).setMsg("校验异常"+strBuilder.toString());
	}
	...
	public JsonResult errorHandler(HttpMessageNotReadableException ex) {
		...
		return new JsonResult(JsonResult.ERROR).setMsg("json数据格式有错");
	}
	...
	public JsonResult errorHandler(MethodArgumentNotValidException ex) {
		...
		return new JsonResult(JsonResult.ERROR).setMsg("校验错误"+sb.toString());
	}
	...
	public JsonResult errorHandler(ValidationException ex) {
		logger.error("校验错误",ex);
		return new JsonResult(JsonResult.ERROR).setMsg("校验错误:"+ex.getMessage());
	}
	...
	public JsonResult errorHandler(RpcException ex) {
		logger.error("远程调用错误",ex);
		return new JsonResult(JsonResult.ERROR).setMsg("远程调用失败:"+ex.getMessage());
	}
	...
	public JsonResult errorHandler(Exception ex) {
		...
		return new JsonResult(JsonResult.ERROR).setMsg(ex.getMessage());
	}
